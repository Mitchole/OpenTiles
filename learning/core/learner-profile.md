---
type: learner-profile
version: 1
created: 2025-12-19
last_updated: 2025-12-19
token_budget: 800
---

# Learner Profile

## Identity
- **Name**: Jake Mitchell
- **Start Date**: 2025-12-19
- **Current Phase**: Pre-Phase 1 (System Setup)

## Experience Level
- **Programming**: Complete beginner
- **Web Development**: None
- **Game Development**: None
- **Technical Background**: [To be filled in]

## Goals

### Primary Goal
Build **OpenTiles**—a Piano Tiles clone with:
1. Full-stack web platform (React frontend, Node.js backend)
2. Unity game implementation (C#)

### Project Locations
- **Learning system**: `OpenTiles/learning/`
- **Web project**: `OpenTiles/dev/web/`
- **Unity project**: `OpenTiles/dev/unity/`

### Learning Objectives
- Master web fundamentals (HTML, CSS, JavaScript)
- Build production React applications
- Create Node.js/Express backends with databases
- Learn C# and Unity basics for game development

### Timeline
- **Target Duration**: 18-24 months
- **Weekly Commitment**: 10-15 hours

## Constraints
- Self-taught (no bootcamp/formal instruction)
- Learning alongside work/life responsibilities
- Must avoid tutorial hell and AI dependency

## Learning Preferences
<!-- Updated by Course Leader as patterns emerge -->
- **Preferred session length**: [TBD]
- **Best learning time**: [TBD]
- **Preferred explanation style**: [TBD]
- **Response to struggle**: [TBD]

## Anti-Patterns to Watch
- Over-reliance on AI for solutions
- Passive tutorial consumption without building
- Skipping JavaScript fundamentals for React
- Avoiding error messages/debugging

## North Star
> Build OpenTiles from scratch, understanding every line of code written.
> The goal is competency, not just completion.
